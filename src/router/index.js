...
import {logout} from '../services/user.service'//, refreshTokenM
// import VueCookies from 'vue-cookies'
import Dashboard from "../views/policeRecords/Dashboard/Dashboard";
...
import RecordsManagement from "../views/policeRecords/records/RecordsManagement";
import login from "../views/policeRecords/authentication/login";
...
const routes = [
    ...
    //     path: "/",
    //     name: "/",
    //     redirect: "/dashboards/agent-dashboard",
    // },
    {
    ...
    {
        path: "/policeRecords/login",
        name: "Login",
        component: login,
    },
    {
        path: "/policeRecords/records",
...
router.beforeEach((to, from, next) => {
    ...
    const publicPages = ['/policeRecords/login']
    const authRequired = !publicPages.includes(to.path)
    const loggedIn = localStorage.getItem('user')
    // const token = VueCookies.isKey('token')
    // const refreshToken = VueCookies.isKey('refresh_token')
    if (authRequired && !loggedIn) {
        return next('/policeRecords/login')
    }
    ...
    if (loggedIn) {
        try {
            ...
            if (to.path === '/') {
                return next('/dashboards/dashboard')
            }
    ...
    if (to.path === '/policeRecords/login' && loggedIn) {
        return next('/')
    }
